...
const configSchema = Joi.object({
  ...
  MONGODB_URI: Joi.string().required().default('mongodb://localhost:27017/amazon_clone_payment'),
...
if (process.env.NODE_ENV !== 'test') {
  mongoose
    ...
    .then(() => {
      logger.info('Connected to MongoDB successfully for Payment Service.');
      app.listen(config.PORT, () => {
        logger.info(`Payment Service is running on port ${config.PORT}`);
      });
    })
    .catch((err) => {
